const { timer_broadcast, checa_broadcast } = require('../../../core/events/broadcast')
...
module.exports = async ({ client, interaction, bot }) => {
    ...
    if (client.decider(bot?.transmission.status, 0))
        return interaction.reply({
            content: ":o: | Um broadcast está ativo no momento, não é possível ativar dois ao mesmo tempo!",
            ephemeral: true
        })
    ...
    interaction.reply({
        content: `:satellite: | O Broadcast entre canais está ativo, agora enviarei mensagens para o canal <#${bot.transmission.id_broad}>\nUse este canal para receber mensagens do canal definido e conversar com outros usuários remotamente!`,
        ephemeral: true
    })
