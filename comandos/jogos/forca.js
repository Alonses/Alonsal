...
function verifica_palavra(client, interaction, user, entrada) {
    ...
    if (entrada === games[interaction.user.id].word || games[interaction.user.id].descobertas.replaceAll("`", "").replaceAll(" ", "") === games[interaction.user.id].word) {
        interaction.reply({ content: `${client.emoji(emojis_negativos)} ${client.tls.phrase(user, "game.forca.acertou")} \`${games[interaction.user.id].word}\``, ephemeral: client.ephemeral(user?.conf.ghost_mode, 0) })

        games[interaction.user.id].finalizado = true
    } else if (entrada.length > 1 || games[interaction.user.id].erros >= 7) {
        interaction.reply({ content: `${client.emoji(emojis_dancantes)} ${client.tls.phrase(user, "game.forca.errou")} \`${games[interaction.user.id].word}\``, ephemeral: client.ephemeral(user?.conf.ghost_mode, 0) })
