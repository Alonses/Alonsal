...
module.exports = async function({client, message, args, playlists, nome_faixas, repeteco, feedback_faixa, atividade_bot, tocar, id_canal}){
    ...
    if(typeof playlists !== "undefined"){
        ...
        if(args.length !== 2){
            message.reply(":octagonal_sign: | "+ musicas[7]["aviso_1"]);
            return;
        }
        ...
        if(isNaN(rm_faixa) && args[1] !== "all"){
            message.reply(musicas[7]["aviso_2"]);
            return;
        }
        ...
        if(rm_faixa !== 1){
            if(args[1] === "all" && queue_local.length >= 2){
                ...
                message.reply(":wastebasket: | "+ musicas[7]["removido_1"]);
            }else{
                ...
                message.reply(":wastebasket: | "+ musicas[7]["removido_2"] +" [ `"+ faixa_removida +"` ] "+ musicas[7]["removido_3"]);
            }
        }else{
            message.reply(musicas[7]["aviso_3"]);
            return;
        }
