...
module.exports = async (message, client, args, playlists, nome_faixas, atividade_bot, repeteco, feedback_faixa, condicao_auto) => {
    ...
    if(!Vchannel)
        return message.reply(musicas[0]["aviso_1"]);
    ...
    if(cond_auto !== "end" && condicao_auto !== "updt"){
        if(!ytdl.validateURL(queue_local[0])){
            await message.reply(":octagonal_sign: | "+ musicas[4]["aviso_1"]);
            client.queue.shift();
    ...
    function tocar_faixa(id_canal){
        ...
        if(fator_renatos > 0){
            try{
                ...
                message.reply(":no_entry_sign: | "+ musicas[4]["error_1"] +" [ "+ queue_interna[0] +" ]");
            }
        ...
        dispatcher.on("error", () => {
            if(typeof queue_interna[0] !== "undefined"){
                message.reply(musicas[4]["error_2"] +" [ <"+ queue_interna[0] +"> ]\n"+ musicas[4]["skip"]);
        ...
        dispatcher.on("finish", () => {
            ...
            if(queue_interna.length > 0 && cond_auto !== "end"){
                ...
                message.reply(musicas[4]["sem_faixas"] +" "+ emoji_dancando);
            }

            inativo = setTimeout(() => {
                message.reply(musicas[4]["desconectado"]);
