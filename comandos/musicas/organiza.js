module.exports = async function({client, message, args, playlists, nome_faixas, repeteco, feedback_faixa, atividade_bot, tocar, id_canal}){
    ...
    if(message.content == ".asrs"){
        ...
        message.reply(":recycle: | "+ musicas[6]["reiniciando"]);
        return;
    }
    ...
    if(message.content === ".asrp"){
        ...
        if(repeteco_ === 0){
            ...
            message.reply(":repeat: "+ musicas[6]["repeteco_1"]);

            if(queue_local.length < 6 && queue_local.length != 0)
                message.reply(musicas[6]["playlist_pequena"]);
            else if(queue_local.length == 0)
                message.reply(musicas[6]["sem_faixas"]);
            else
                message.reply(musicas[6]["anuncio_faixas"]);
        }else{
            await repeteco.set(id_canal, 0);
            message.reply(":arrow_forward: "+ musicas[6]["repeteco_2"]);
        }
