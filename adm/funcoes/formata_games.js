module.exports = (objeto_anunciado, plataforma, idioma_definido) => {
    ...
    if (objeto_anunciado[0].tipo !== "dlc" && objeto_anunciado[0].urgencia !== "u"){
        ...
        if (objeto_anunciado[1]){
            texto_formatado = `Os Games _\`${objeto_anunciado[0].nome}\`_ & _\`${objeto_anunciado[1].nome}\`_ estão gratuitos até o dia \`${objeto_anunciado[0].expira}\` por lá\n\nResgate ambos antes da data para poupar \`R$${valor_total}\` e garantir uma cópia em sua conta ${plataforma}`

            if (idioma_definido === "en-us")
                texto_formatado = `The Games _\`${objeto_anunciado[0].nome}\`_ & _\`${objeto_anunciado[1].nome}\`_ are free until the day \`"${objeto_anunciado[0].expira}\` over there\n\nRedeem both before date to save \`R$${valor_total}\` and get a copy in your ${plataforma} account`
        }
        ...
        if (objeto_anunciado[1]){
            texto_formatado = `As DLC's _\`${objeto_anunciado[0]}\`_ & _\`${objeto_anunciado[1].nome}\`_ estão gratuitas até o dia \`${objeto_anunciado[0].expira}\` por lá\n\nResgate ambas antes da data para poupar \`R$${valor_total}\` e garantir uma cópia em sua conta ${plataforma}`

            if (idioma_definido === "en-us")
                texto_formatado = `The DLC's _\`${objeto_anunciado[0].nome}\`_ & _\`${objeto_anunciado[1].nome}\`_ are free until the day \`"${objeto_anunciado[0].expira}\` over there\n\nRedeem both before date to save \`R$${valor_total}\` and get a copy in your ${plataforma} account`
        }
