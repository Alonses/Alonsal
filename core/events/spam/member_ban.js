...
module.exports = async ({ client, message, guild, user_messages, strike_aplicado, indice_matriz, mensagens_spam, user, user_guild, guild_bot }) => {
    ...
    if (!await client.execute("permissions", { message, id_user: client.id(), permissions: [PermissionsBitField.Flags.BanMembers] }) || guild_bot.roles.highest.position <= user_guild.roles.highest.position)
