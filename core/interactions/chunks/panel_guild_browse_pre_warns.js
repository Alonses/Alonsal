...
module.exports = async ({ client, user, interaction, dados, pagina_guia }) => {
    ...
    if (user_notes[indice_start + 6])
        botoes.push({ id: "pre_warn_user_verify", name: { tls: "status.proxima" }, emoji: client.emoji(41), type: 2, data: `10|${user_notes[0].uid}.${pagina_guia}` })
    else if (user_notes.length > 5)
        botoes.push({ id: "pre_warn_user_verify", name: { tls: "menu.botoes.retornar" }, emoji: client.emoji(57), type: 2, data: `11|${user_notes[0].uid}` })

    for (let x = indice_start; x < user_notes.length; x++) {

        botoes.push({ id: "pre_warn_user_verify", name: `${x + 1}°`, emoji: client.defaultEmoji("guard"), type: 0, data: `9|${user_notes[x].uid}.${user_notes[x].timestamp}` })
