...
module.exports = async ({ client, user, interaction, pagina_guia }) => {
    ...
    if (pagina === 0) {
        ...
        botoes.push(
            { id: "guild_reports_button", name: { tls: "manu.painel.reports_externos", alvo: user }, type: client.execute("functions", "emoji_button.type_button", guild?.conf.reports), emoji: client.execute("functions", "emoji_button.emoji_button", guild?.conf.reports), data: "1" },
            { id: "guild_reports_button", name: "AutoBan", type: 1, emoji: client.emoji(41), data: "20" },
            { id: "guild_reports_button", name: { tls: "menu.botoes.ajustes", alvo: user }, type: 1, emoji: client.emoji(41), data: "21" }
        )
    } else if (pagina === 1) {
        ...
        botoes.push(
            ...
            { id: "guild_reports_button", name: { tls: "menu.botoes.exclusao", alvo: user }, type: 1, emoji: client.emoji(13), data: "6" }
        )
    } else if (pagina === 2) {

        embed.setDescription(client.tls.phrase(user, "mode.report.funcionamento_aviso_entradas"))
        botoes.push(
            { id: "guild_reports_button", name: { tls: "mode.report.aviso_de_entradas", alvo: user }, type: client.execute("functions", "emoji_button.type_button", guild?.reports.notify), emoji: client.execute("functions", "emoji_button.emoji_button", guild?.reports.notify), data: "3" },
            { id: "guild_reports_button", name: { tls: "mode.anuncio.cargo", alvo: user }, type: 1, emoji: client.defaultEmoji("role"), data: "5", disabled: !membro_sv.permissions.has(PermissionsBitField.Flags.ManageRoles) },
            { id: "guild_reports_button", name: { tls: "mode.report.canal_de_avisos", alvo: user }, type: 1, emoji: client.defaultEmoji("channel"), data: "4" },
        )
    }
    ...
    client.reply(interaction, {
        content: "",
        embeds: [embed],
        components: [client.create_buttons(botoes, interaction), client.create_buttons([{ id: "return_button", name: { tls: "menu.botoes.retornar", alvo: user }, type: 0, emoji: client.emoji(19), data: pagina < 1 ? "panel_guild.1" : "panel_guild_external_reports" }], interaction)],
        flags: "Ephemeral"
    })
}