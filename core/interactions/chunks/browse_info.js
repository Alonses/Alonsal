...
module.exports = async ({ client, user, interaction, caso }) => {
    ...
    if (pagina === 0) // Página inicial
        botoes.push(
            { id: "browse_info", name: { tls: "menu.botoes.estatisticas" }, type: 0, emoji: client.defaultEmoji("metrics"), data: 3 },
            { id: "browse_info", name: { tls: "inic.inicio.suporte" }, type: 0, emoji: client.emoji(25), data: 1 },
            { id: "browse_info", name: { tls: "manu.data.links_externos" }, type: 0, emoji: client.emoji(32), data: 2 },
        )
    else if (pagina === 1)
        botoes.push(
            { id: "return_button", name: { tls: "menu.botoes.retornar" }, type: 2, emoji: client.emoji(19), data: "browse_info" },
        ...
        embed.setDescription(`${client.defaultEmoji("metrics")} ${client.tls.phrase(user, "manu.info.guia_estatisticas")}`)
            .addFields(
                ...
                },
                {
                    ...
                    value: `${client.emoji("icon_slash_commands")} **${client.tls.phrase(user, "manu.info.comandos_usados")}: **\`${client.execute("locale", { valor: bot.cmd.ativacoes })}\`\n${client.emoji("mc_esmeralda")} **Bufunfas: **\`${client.execute("locale", { valor: bot.bfu.gerado })}\``,
                    inline: true
                },
                {
                    name: "⠀",
                    value: `:mouse_three_button: **${client.tls.phrase(user, "manu.info.botoes_clicados")}: **\`${client.execute("locale", { valor: bot.cmd.botoes })}\`\n${client.emoji("mc_nether_star")} **${client.tls.phrase(user, "manu.info.xp_gerado")}: **\`${client.execute("locale", { valor: bot.exp.exp_concedido })}\``,
                    inline: true
                },
                {
                    name: "⠀",
                    value: `:card_box: **${client.tls.phrase(user, "manu.info.menus_abertos")}: **\`${client.execute("locale", { valor: bot.cmd.menus })}\`\n${client.emoji("icon_voice_channel")} **${client.tls.phrase(user, "manu.info.canais_gerados")}:** \`${client.execute("locale", { valor: bot.cmd.voice_channels })}\``,
                    inline: true
                },
                {
                    name: `${client.defaultEmoji("calendar")} **${client.tls.phrase(user, "manu.info.historico")}**`,
                    value: `${client.emoji("icon_slash_commands")} **${client.tls.phrase(user, "manu.info.comandos_usados")}: **\`${client.execute("locale", { valor: bot.persis.commands })}\`\n:globe_with_meridians: **${client.tls.phrase(user, "mode.network.servidores")}: **\`${client.execute("locale", { valor: client.guilds().size })}\`\n:name_badge: **${client.tls.phrase(user, "manu.info.spams_freados")}: **\`${client.execute("locale", { valor: bot.persis.spam })}\`\n:link: **${client.tls.phrase(user, "mode.spam.links_suspeitos")}:** \`${links_suspeitos.length || 0}\`\n${client.emoji("mc_esmeralda")} **Bufunfas: **\`${client.execute("locale", { valor: bot.persis.bufunfas })}\``,
                    inline: true
                },
                {
                    name: "⠀",
        ...
        botoes.push(
            { id: "return_button", name: { tls: "menu.botoes.retornar" }, type: 2, emoji: client.emoji(19), data: "browse_info" },
            { id: "browse_info", name: { tls: "menu.botoes.atualizar" }, type: 0, emoji: client.emoji(42), data: 3 },
        )

        if (games_free.length > 0) // Jogos gratuitos disponíveis para coleta
            botoes.push({ id: "free_games", name: { tls: "menu.botoes.ver_jogos_free" }, type: 0, emoji: client.emoji(29), data: 0 })
    }
