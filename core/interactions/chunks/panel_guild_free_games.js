...
module.exports = async ({ client, user, interaction }) => {
    ...
    botoes.push(
        { id: "guild_free_games_button", name: { tls: "manu.painel.anuncio_games" }, type: client.execute("functions", "emoji_button.type_button", guild?.conf.games), emoji: client.execute("functions", "emoji_button.emoji_button", guild?.conf.games), data: "1" },
        { id: "guild_free_games_button", name: { tls: "mode.anuncio.cargo" }, type: 1, emoji: client.defaultEmoji("role"), data: "3", disabled: b_cargos },
        { id: "guild_free_games_button", name: { tls: "mode.report.canal_de_avisos" }, type: 1, emoji: client.defaultEmoji("channel"), data: "4" }
    )
    ...
    if (guild.games.channel && guild.games.role) // Mostrado apenas quando um canal e um cargo está definido para o anúncio de games no servidor
        botoes.push({ id: "guild_free_games_button", name: { tls: "menu.botoes.anunciar_agora" }, type: 1, emoji: client.emoji(6), data: "2" })

    client.reply(interaction, {
        content: "",
        embeds: [embed],
        components: [client.create_buttons(botoes, interaction, user), client.create_buttons([{ id: "return_button", name: { tls: "menu.botoes.retornar" }, type: 0, emoji: client.emoji(19), data: "panel_guild.1" }], interaction, user)],
        flags: "Ephemeral"
    })
}